...
### Changed
- **终端标题去掉空闲 ✳ 前缀 + busy 动画换成方块波浪**（TS+Rust 双实现）：空闲标题由 `✳ DeepDive` 变为纯 `DeepDive`（去掉静态 emoji 前缀，用户要求）；busy 动画由 2 帧盲文 `⠂/⠐`（960ms/帧）换成与 `Running` 组件同款的 6A 方块波浪 `▁▂▃▄▅▆▇` 12 帧 × 80ms（一个完整波周期 = 原 960ms 预算，接近 Running 的 90ms/帧，tab 与输入行动画节奏一致）。TS `src/terminal-title.ts` + Rust `deepdive-tui/src/terminal_title.rs`（`TITLE_ANIMATION_FRAMES`/`TITLE_ANIMATION_INTERVAL_MS`/`buildTerminalTitle`/`title_string`），`TITLE_STATIC_PREFIX` 常量删除；测试同步（TS wraps 断言 2→12 帧、Rust 断言字符更新）；codemap ui 模块及 terminal-title 单元描述回写
