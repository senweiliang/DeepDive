...
### Fixed
- **远程控制：桌面端二维码反复出现在历史对话中（手机远程控制回归）**：二维码块原先作为 `staticItems` 尾插条目渲染，而 `<Static>` 按 index 追加、只打印「从上次打印位置起的新项」——尾插条目每来一条新消息 index 后移，永远落在未打印区 → 每条消息都重印一次二维码，终端输出混乱。改为**一条 `remote:true` 的普通 Message**（`content`=URL、`qr`=二维码字符串）：在 `remoteStatus.running` 翻转时经 effect 追加进 `messages` 一次，MessageItem 渲染专用块，index 固定不再重印。横幅消息客户端专属——`stripNonApiFields` 过滤（不进模型）、持久化 effect 跳过（`-r` 恢复不重放死掉的 URL）、手机快照过滤（桌面 UI）
- **手机端 tool 输出未截断**：页面里 tool 消息按 4 行截断，但桌面 `ToolResult` 是 `RESULT_PREVIEW_LINES=3` + `… +N lines`，且截断分支只在 `role==="tool"` 时生效。改为与桌面完全对齐：3 行预览 + `… +N lines` 标记（含 4 空格续行缩进）；顺带修复快照整体重绘时把用户滚动位置顶回顶部的问题（重绘前记录 wasNearBottom/scrollTop，非底部则恢复原位置）
  - 验证：`src/__tests__/remote-page.test.ts` 从 5 例扩到 12 例——新增 7 例 DOM 桩模拟（真实页面 JS 提取出来跑在假 DOM 上：消息顺序、tool 截断精确断言、toolcall/思考折叠、XSS 转义、流式尾渲染、滚动保持/自动跟随）；并做了真实链路模拟（起 harness 服务器 + 连 127.0.0.1 + GET 页面 + SSE 取快照 + POST 手机消息 + 渲染响应页，tool 50 行 → `⎿ 第1行/第2行/第3行/… +47 lines`、重渲染稳定无累积、手机消息到达 sendMessage）
...
### Changed
- **分类器对齐 Claude Code：紧凑 transcript 投影 + 两阶段判定**（TS `src/tools/classifier.ts` + Rust `deepdive-core/src/tools/classifier.rs` 双实现，行为一致）：修复 fcb16020 会话里同一 `browser-harness --doctor` 在模型路径上 2 block / 2 allow 的不稳定
  - **输入投影**：分类器输入从「单条命令 + 极简 platform/cwd」改为 Claude 式紧凑 JSONL transcript（`buildClassifierMessage` → `buildClassifierTranscript`）——只含真实 user 文本（跳过 `meta`/`error`）与 assistant 工具调用，工具经 `toClassifierToolInput` 只暴露安全相关字段（bash→`command`、read_file→`file_path`、write_file/edit_file→`file_path: new_string`、glob/grep→`{pattern,path}`、web_search→`query`、web_fetch→`url`、agent→`subagent_type: prompt`）；**assistant 自由文本与 tool 结果被刻意排除**（防提示注入借道历史内容，对齐 Claude 官方「分类器去掉工具输出」）；单条 4k / 总 16k 截断、从最新尾部保留完整行；待审命令 `{"bash":…}` 追加在 `<transcript>` 末尾恰好一次
  - **两阶段判定**（`FAST_STAGE_SUFFIX` / `REVIEW_STAGE_SUFFIX`）：FAST 只输出 `allow`/`review`（16 token）→ 明确 `allow` 直接返回；其余进入 REVIEW 三分类 + 理由（160 token）；任何解析失败 / API 错误回落 `ask`（fail-closed，永不因故障误放行或误拦截）；TS 换用 `fetchResilient`（2 次尝试），Rust 换用 `send_resilient`
  - **确定性快速路径扩面**：临时 `set PATH=… && <tool> --doctor|--help|--version|-h|-V` 自检放行（进程级 `set` ≠ 持久 `setx`）；`isSafeChromeForTestingLaunch` 放行 Chrome for Testing 受限启动（「可能占住 shell」是可用性问题不是安全风险）；`isSafeWorkspaceDelete` 只放行工作区内相对路径、无 `..`/无通配符/无 `&|;<>` 的单文件 `del`；新增 `contextualHeuristicClassify`——`taskkill /f /im chrome.exe …` 清理命令仅在 transcript 显示本轮启动过专用 Chrome 时放行（无轨迹的按进程名杀进程不进白名单）；顺带补齐 Rust 落后于 TS 的 `cd /d <path> &&` 剥离与 Windows 只读命令集合 parity
  - **prompt 语义放宽**：持久系统配置不再天然 block——显式请求 + 窄范围 + 目标可信且符合用户意图时 allow（含 `setx PATH`），范围/授权不清 ask，劫持可执行解析才 block；Windows 平台段补 `set NAME=value && command` 语义说明
  - **接线**：`App.tsx` auto 模式 bash 审批把完整 `history` 传入 `classify`；Rust `engine.rs` `gate_tool_interactive` 改传 `&session.history`，删除旧的 `most_recent_non_meta_user`/`recent_user` 接口
  - **回归覆盖**：TS 72 例（+transcript 投影断言、两阶段 mock：fast allow 只 1 次调用、review 路径 2 次调用、contextual chrome 启动→清理、`del ..\x`/`del *.ts`/`set PATH && unknown-tool run` 反例）；Rust 10 例；类型检查 + TS/Rust 全量测试通过
  - 文档：codemap safety/command-classifier 升 ✅（新建 `spec.md`、`adr/0002-classifier-transcript-and-two-stage.md`、补 `adr/README.md` 索引）
