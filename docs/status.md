...
## 已完成
- [x] **手机远程控制移动端 UI 第五轮补丁——折叠箭头修复**（`src/remote/page.ts`，用户反馈"三角形丑、箭头不居中、旋转不对"）：三处折叠箭头（思考过程/工具结果/命令输出）从 Unicode `▶` 字符改**内联 SVG chevron**（`chevronEl()` 公共函数，12px，`currentColor`，跨平台一致零外部资源）；toolcall 胶囊藏掉浏览器默认 details 三角形（`list-style:none` + `::-webkit-details-marker{display:none}`）——胶囊只剩蓝圆点+工具名；箭头容器从 `inline`/`inline-block` 改 **`inline-flex` + align-items:center**（修复 transform 对 inline 不生效导致的不旋转 + SVG baseline 偏移不居中）。真实浏览器 CDP 验证：箭头中心在 summary 垂直中线（13/27、19/39），展开后 `transform=matrix(0,1,-1,0,0,0)`=rotate(90deg) 生效。全量 332 测试 + typecheck 绿
- [x] **手机远程控制移动端 UI 第五轮打磨——UI 元素细节**（`src/remote/page.ts`，按 frontend-design skill 原则"把大胆用在一个地方，其余克制"）：发送按钮加**内联 SVG 发送图标**（no emoji、零外部资源，配文字"发送"）；用户气泡加**柔和阴影**（`0 2px 8px rgba(0,0,0,.25)`，pending 态去除）；toolbox 折叠**展开时内容淡入下移**（`tbIn` 160ms ease-out）；思考过程折叠加**箭头指示**（`details.thinkbox`，`▶思考过程`，展开旋转 90°）；header 顶部加**1px 品牌渐变细线**（`linear-gradient(transparent→accent→transparent)`，呼应桌面 splash）；连接状态点加**光晕**（on 绿光 / busy 琥珀光晕）；toolcall 圆点加**微光**；空会话引导改**品牌终端风**（波浪 `〰` 浮动 + `deepdive@remote:~ 提示符胶囊 + 引导文字）。全部新增动画纳入 `prefers-reduced-motion`。真实浏览器 CDP 逐项验证（消息态 + 空状态），全量 332 测试 + typecheck 绿
- [x] **安装 Anthropic 官方 frontend-design skill**（`~/.deepdive/skills/frontend-design/SKILL.md`，来自 `anthropics/claude-code` 仓库 `plugins/frontend-design/`，~50 行设计哲学：两遍法——先出 token 系统设计计划（color/type/layout/signature）再落地，打破"AI slop"模板默认。无 `${CLAUDE_PLUGIN_ROOT}` 引用无需替换；经 7897 代理下载）
- [x] **手机远程控制移动端 UI 第四轮打磨——深海品牌配色**（`src/remote/page.ts`，按 frontend-design skill 落地，用户反馈"配色细节难看"）：整体配色从 GitHub 暗色默认（`#0d1117` 灰蓝）改为**深海蓝调**（承接 DeepDive 品牌）：bg `#0a0f1c` / panel `#101828` / border `#1c2740` / fg `#dbe2ec` / dim `#7e8aa3` / accent `#6ab3f7` / user 气泡 `#173a63` / ok `#4ade80`（声呐绿）/ warn `#f5c65c` / err `#ef6a80`；全部硬编码 hex 收敛为 token（骨架屏 `#26313f`→`#1b2740`、hover `#7cc0f4`→`#7cbcf7`、toast `#d73a49`→`#c2455a`、按钮字 `#0b0f14`→`#08111f`、jumpPulse 阴影同步）。签名元素：标题改等宽终端字（`ui-monospace` + letter-spacing）、用户气泡改「对话尾巴」圆角 `16px 16px 4px 16px`。真实浏览器 CDP computed style 逐项确认新色生效，全量 332 测试 + typecheck 绿
- [x] **手机远程控制移动端 UI 第三轮打磨——工具显示渐进披露 + 动效 + 配色**（`src/remote/page.ts`，用户反馈"工具执行显示碍眼、文本太多、动效不好"）：toolcall 从平铺长文本 `→ name(args前100字)` 改**可展开胶囊**（默认只显工具名、参数点开才看）；tool 结果 / bash 输出从平铺 3-6 行预览改 **`details.toolbox` 折叠卡片**（默认收起只占一行：`▶工具结果 · N 行`，点击展开全文，替代平铺长文本）；新消息入场**轻微上浮淡入**（180ms ease-out，仅非首快照的新增消息，streaming 重建不加）；配色按 skill 语义色 tokens——工具名/胶囊文字/卡片标题改**品牌蓝 `--accent`**（动作语义）、说明文字（行数/箭头）保留灰、思考过程琥珀，正文亮白，形成「调用=蓝 / 结果=蓝名灰文 / 思考=琥珀」层级。DOM 桩测试更新 2 例（toolbox 折叠 + toolcall 胶囊），全量 332 测试 + typecheck 绿；真实浏览器 CDP 验证（computed style 确认颜色、展开交互、动画规则就位）
- [x] **手机远程控制移动端 UI 第二轮打磨**（按 ui-ux-pro-max skill 规则对照落地，`src/remote/page.ts`）：发送中按钮从「文字变…」改为 **CSS spinner**（`#send.loading::before` 旋转圆环，`reduced-motion` 下停转）；全站补 `touch-action:manipulation`（消除移动端 300ms tap delay）；「回到底部」按钮 40px→**44px 触控目标**；发送/回底按钮补桌面 hover 态（`#7cc0f4` 提亮）；连接状态点加 `role="status" aria-live="polite"`（屏幕阅读器可播报连接状态）。真实浏览器 CDP 渲染冒烟（mock SSE 快照：已连接 + 消息 + tool 结果渲染正常）通过
- [x] **远程控制：手机端看不到回复最后几个字（流式尾部丢失，TS）**：`pushSnapshot` 节流（150ms）在 pending 定时器存在时**直接丢弃**后续快照、只推窗口内第一帧——流式 ~40ms 一帧、几乎每个窗口都挤进多帧，流结束的最终快照（`isStreaming:false` + 完整消息）一旦落在 pending 窗口内被吞，手机端永远卡在旧 streaming 文本上（表现为"想到"之后"什么随时说"看不到）。修复 `src/remote/server.ts`：pending 期间每次调用都**覆盖 `pendingSnap` 保留最新**，窗口边界推最新快照。新增节流用例（窗口内 3 帧 → 推送最后一帧内容），server 测试 7→8 例，全量 331 例 + typecheck 绿
- [x] **权限规则建议：危险包装器固定到具体子命令**（TS+Rust 双实现，用户口径）：`sudo`/`cmd`/`powershell`/`sh` 等不再整类隐藏 "Allow always"——`cmd /c dir` 生成 `Bash(cmd /c dir:*)`、`sudo apt install` 生成 `Bash(sudo apt:*)`、`timeout 30 node` 生成 `Bash(timeout 30 node:*)`；首个可约束子 token 命中破坏性/危险黑名单（新增 `DESTRUCTIVE_SUBCOMMANDS`：rm/del/tee/setx/systemctl 等）或无可约束 token 时仍 veto（`sudo rm -rf /`、`sh -c "x"`、`cmd /c del` 无建议）。TS permissions 35 例 / Rust 13 例全过；全量 TS 330 例绿
- [x] **手机远程控制移动端 UI 优化**（按 ui-ux-pro-max skill 设计检索落地，`src/remote/page.ts`）：快照驱动协议不变，手机端从整体重绘改**增量渲染**（消息签名 diff——未变节点原地复用、streaming 尾部只更新文本不重建 wrapper，streaming 期间不再整页闪动/滚动跳动）；首快照前骨架屏、空会话引导；发送失败 alert→toast（失败不清空输入）、滚出底部浮出「↓ N 条新消息」跳底按钮、排队消息虚线 pending 气泡；发送按钮 44px + 按压反馈、input 16px 防 iOS 缩放、safe-area-top、`prefers-reduced-motion`、streaming 状态点琥珀脉冲。DOM 桩测试 12→18 例，真实浏览器 CDP 冒烟（390×844 视口渲染 + 发消息链路）通过，全量 325 测试 + typecheck 绿
- [x] **分类器对齐 Claude Code：紧凑 transcript 投影 + 两阶段判定**（TS+Rust 双实现，修复 fcb16020 会话 `--doctor` 2block/2allow 漂移）：分类器输入从「单条命令+极简环境」改为 Claude 式紧凑 JSONL transcript——只含真实 user 文本 + assistant 工具调用（`toClassifierToolInput` 逐工具投影安全字段），排除 assistant 自由文本与 tool 结果（防提示注入借道历史）；待审命令 `{"bash":…}` 追加在 `<transcript>` 末尾恰好一次。模型路径改两阶段：FAST（16 token 只出 allow/review）明确 allow 即返回，其余进 REVIEW（160 token 三分类+理由），任何错误回落 ask。heuristic 补窄范围确定性放行：临时 `set PATH=… && (--doctor|--help|--version)` 自检、Chrome for Testing 受限启动（「占住 shell」非安全风险）、工作区内相对路径单文件 `del`；新增 `contextualHeuristicClassify`——`taskkill /im chrome.exe` 清理命令只有 transcript 显示本轮启动过专用 Chrome 才放行（无轨迹杀进程不白名单化）。prompt 同步放宽 `setx PATH` 判定（显式请求+窄范围+可信才 allow，劫持解析才 block）。TS 75 例 / Rust 11 例全过（反例在列：`del ..\x`、`del *.ts`、`set PATH && unknown-tool run`）；Rust `recent_user` 旧接口删除。codemap：safety/command-classifier 升 ✅（新建 spec.md + adr/0002）
